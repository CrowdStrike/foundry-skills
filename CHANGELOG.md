...
- **Function execution, testing, and debugging (CLI 2.1.0+)** — The `functions-development` skill now covers `foundry functions exec`, `test`, `logs`, and the debugging workflow for deployed functions. Documents the deploy-first rule (exec/test run the deployed artifact, not local code), request-data confirmation before exec, handler/function disambiguation from the manifest, the `tests.yml` schema for integration test cases, and a code-review checklist for logging adequacy, sensitive data, and schema coverage. The CLI guard hook enforces `--no-prompt` on these new commands. All gated on CLI 2.1.0+; users on 2.0.x are informed and offered an upgrade when they try to use these commands.
