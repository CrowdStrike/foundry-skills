...
- **The Detection Triage Agent example is consistent again.** Its `agents create` command used `--output-format json` while the manifest and on-disk listings still showed `json_with_schema`, an `output_schema.json` file, and `model: ""` (a combination the new pitfall row says fails on Bedrock Claude).
...
- **Transient faas-gateway 404s from UI cloud-function calls** — `debugging-workflows` now notes that a UI page calling a cloud function through foundry-js shows repeated `404`s on `/api2/faas-gateway/entities/execution/v1` in the browser console while it polls for the result; these are not failures if the data ultimately loads, so they should not send you debugging a working call.
- **`foundry apps sync` flags and the ID-refill friction** — `development-workflow` (advanced-patterns) now documents that `sync` requires `--deployment-version` under `--no-prompt`, defaults its target to a new `AppName/` subdirectory unless you pass `-d .` (with `--replace-all` to sync in place), and takes `--app-id` when the manifest's is blank. It also spells out that committing a manifest with blanked IDs (the `foundry-sample-*` pattern) fights local tooling: `foundry functions exec` fails with `app_id not found in manifest` and `deploy` needs the IDs to target the existing app, so re-fill them before working locally and blank them again before committing. The refill syncs into a scratch directory and merges only the IDs with `yq`, because `sync -d . --replace-all` overwrites every undeployed edit in the working copy.
- **Attributing AgentWorks executions to an agent** — `functions-falcon-api` (advanced-patterns) now documents that an `aw_agent` root span carries the agent's ID in `aw_agent.id`, so `attributes.aw_agent.id:'<agent id>'+span_type:'aw_agent'` returns an agent's runs and the query total is its execution count. The similar-looking `attributes.aw_agent.agent_id` silently matches nothing, the span's `name` is always `Agent request`, there is no version ID (so no per-version counts), and run status lives on the `aw_agent_response` child span. Matching on `aw_agent.definition.name` is discouraged because AgentWorks agent names are not unique per CID.
...
- **A missing collection object is not a 404 in FalconPy 1.6.5 and earlier.** The API answers with an empty body and FalconPy's error check calls `.get()` on those bytes, so `GetObject` returns a synthetic 500, `'bytes' object has no attribute 'get'`. `collections-development` and the `python-patterns` `get_incident` example now treat that message as not-found (falconpy#1509 fixes it upstream), and warn against the opposite shortcut of treating every non-bytes reply as missing, which lets a transient error erase a record on write-back.
- **There is no `falcon.theme()` in foundry-js.** `ui-development` and its theming reference called it; `connect()` already puts `theme-light` or `theme-dark` on `<html>` (from `falcon.data.theme`), and `@crowdstrike/falcon-shoelace` styles off exactly those classes, so no theme code is needed.
- **`api_path` has no `{param}` placeholders and function descriptions are not limited to 100 alphanumeric characters.** The Python FDK router matches the path exactly; the CLI checks only length, 1024 characters for a function description and 512 for a handler's.
- **The asset CDN serves foundry-js 0.20.0 only.** `@0.21.0` and `@0.22.0` return `403` from `assets.foundry.crowdstrike.com`; `ui-development`'s blueprint reference shows vendoring the self-contained npm `dist/index.js` for a no-build page that needs a newer version.
