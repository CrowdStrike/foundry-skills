...
- **Charlotte AI AgentWorks APIs from functions** — `functions-falcon-api` shows how to call `/agentic-studio/*` endpoints, which FalconPy doesn't wrap, and how to count an agent's executions from its trace spans. The `charlotte-ai-agent-definition` scopes are added to the scope reference, and `api-integrations` covers writing a minimal spec for other Falcon APIs FalconPy doesn't wrap.
