...
- **`foundry apps sync` flags and the ID-refill friction** — `development-workflow` (advanced-patterns) now documents that `sync` requires `--deployment-version` under `--no-prompt`, defaults its target to a new `AppName/` subdirectory unless you pass `-d .` (with `--replace-all` to sync in place), and takes `--app-id` when the manifest's is blank (observed on CLI 2.1.1). It also spells out that committing a manifest with blanked IDs (the `foundry-sample-*` pattern) fights local tooling: `foundry functions exec` fails with `app_id not found in manifest` and `deploy` needs the IDs to target the existing app, so re-fill them in place with `sync -d . --replace-all` before working locally and blank them again before committing.
