...
- **`foundry apps sync` behavior and the ID-refill friction** — `development-workflow` (advanced-patterns) now notes that `sync` requires `--deployment-version` under `--no-prompt` and writes the synced app into a new subdirectory named after the app rather than updating the current directory. It also spells out that committing a manifest with blanked IDs (the `foundry-sample-*` pattern) fights local tooling: `foundry functions exec` fails with `app_id not found in manifest` and `deploy` needs the IDs to target the existing app, so re-fill them from the deployed version before working locally and blank them again before committing.
