...
cleanup
echo "$$" > "$MARKER"
JSON=$(jq -n '{hook_event_name: "PreToolUse", tool_name: "Skill", tool_input: {skill: "foundry:foundry-development-workflow"}}')
OUTPUT=$(run_hook "$HOOK" "$JSON")
...
# 5.6 — foundry:foundry-development-workflow → no output
cleanup
JSON=$(jq -n '{hook_event_name: "PreToolUse", tool_name: "Skill", tool_input: {skill: "foundry:foundry-development-workflow"}}')
OUTPUT=$(run_hook "$BRIDGE" "$JSON")
