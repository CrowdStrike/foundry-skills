---
name: foundry-collections-development
...
Use `CustomStorage` (Service Class) to access collections from Python functions. Service classes are preferred over the Uber class (`APIHarnessV2`) because the Falcon Foundry functions editor auto-detects OAuth scopes from `from falconpy import CustomStorage`. See [foundry-functions-development/references/python-patterns.md](../foundry-functions-development/references/python-patterns.md) for a complete handler example with Uber class alternative.
